...
2. **Ruby formatting** is commented out in `conform.lua` because your global instructions say Ruby commands run inside Docker. Uncomment the `ruby = { "rubocop" }` line if you want local rubocop.
   - **ruby-lsp over Docker:** for projects whose Ruby lives in a container, `ruby-lsp` runs *inside* the `web` container (host ruby/gems won't match). Wired in `lsp.lua`: a `FileType ruby` autocmd starts `docker compose exec -w <root> -T web bundle exec ruby-lsp` when a `docker-compose.yml` is at the project root; otherwise it uses the host (mason) binary. **Requires** a compose mount exposing the repo at its host path inside the container — e.g. ciphero-web has a local (gitignored via `.git/info/exclude`) `docker-compose.override.yml` adding `".:/Users/michalkowal/Projects/ciphero-web"`. After creating that override, recreate the container (`docker compose up -d web`) so the mount exists, and make sure the stack is **running** before opening a `.rb` file. Caveat: jumping into gem/stdlib sources won't work (those paths only exist in the container).
